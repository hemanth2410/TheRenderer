...
public:
	...
	Node(int id, std::vector<Mesh*> meshPtrs, const DirectX::XMMATRIX& transform, std::string name) noxnd;
	void Draw(Graphics& gfx, DirectX::FXMMATRIX accumulatedTransform) const noxnd;
...
public:
	...
	Model(Graphics& gfx, const std::string fileName, std::string folderName, std::optional<float> manualSourceUnitInMeters = std::nullopt);
	void Draw(Graphics& gfx) const noxnd;
	void ShowWindow(const char* windowName = nullptr) noexcept;
	float GetScale() const noxnd;
	~Model() noexcept;
private:
	static std::unique_ptr<Mesh> ParseMesh(Graphics& gfx, const aiMesh& mesh, const aiMaterial* const* pMaterials, std::string folderName);
