...
namespace Rgph
{
	...
	{
	public:
		void BindShadowCamera(Camera& cam) noexcept
		{
			pShadowCamera = &cam;
		}
			...
			RenderQueuePass(std::move(name))
		{
			...
			pDepthCube = std::make_shared<DepthCubeTexture>(gfx, size, 4);
			...
			AddBind(std::make_shared<Bind::ShadowRasterizer>(gfx, 50, 20.0f, 0.1f));
		...
		{			
			...
			const XMFLOAT3 posData = pShadowCamera->GetPos();
			const auto pos = XMLoadFloat3(&posData);
		...
		{/*
			...
			{
				auto d = pDepthCube->GetDepthBuffer(i);
				d->ToSurface(gfx,false).Save(path + std::to_string(i) + ".png");
			}*/
		}
	private:
